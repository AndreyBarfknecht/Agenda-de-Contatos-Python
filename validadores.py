...
def nome_padrao():
    while True: # loop do nome
            ...
            if not nome_digitado.strip(): # verifica se o nome não está vazio
                interface_usuario.console.print("[yellow]O nome não pode estar vazio!!!")
                continue

            if not re.match(PADRAO_NOME, nome_digitado): # verifica se o nome não tem caracteres especiais
                interface_usuario.console.print("[yellow]O nome contém caracteres especiais. Use apenas letras e espaços")
                continue
            break
...
def telefone_padrao():
    while True: # loop do telefone
        ...
        if not telefone_digitado.strip(): # verifica se não está em branco
            interface_usuario.console.print("[yellow]O telefone não pode estar vazio!!!")
            continue
        #telefone_digitado = input("Qual o telefone --> ") # usuário digita o telefone
        if not re.match(PADRAO_TELEFONE, telefone_digitado): # verifica se está no pradão
            interface_usuario.console.print("[yellow]O telefone contém letras ou caracteres especiais. Use apenas números e espaços")
            continue
        break
...
def email_padrao():
    while True: # loop do email 
        ...
        if not email_digitado.strip():
            interface_usuario.console.print("[yellow]O email não pode estar vazio!!!")
            continue
            #email_digitado = input("Qual o email --> ") # usuário digita o email
        if not re.match(PADRAO_EMAIL, email_digitado):
            interface_usuario.console.print("[yellow]Formato de e-mail inválido. Por favor, tente novamente.")
            continue
        break
